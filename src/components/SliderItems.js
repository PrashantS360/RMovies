...
const SliderItems = (props) => {
    ...
    const { sliderNo, type, title, setProgress } = props;
    const [slideIndex, setSlideIndex] = useState(0);
    let oneSlideItems = 4;

    const getItem = async () => {
        setProgress(30);
        let response = await fetch(`${process.env.REACT_APP_BACKEND_URI}/getmovies`, {
            method: "GET",
            headers: {
        ...
        for (let i = 0; i < json.length; i++) {
            if (json[i].show.type.toLowerCase() === type.toLowerCase()) {
    ...
    return (
        <div className='shadow-md shadow-gray-500'>
            ...
            <div className="header p-3 px-7 flex justify-between items-center border-b-[1px] border-gray-300 mx-2">
                ...
                <Link to={`/search/${type}`} className="view bg-util px-4 py-2 text-sm rounded-sm font-semibold text-white">
                    VIEW ALL
                </Link>
            </div>
            <div className="slideshow-container relative mx-2 mb-2 flex items-center justify-evenly">
                ...
                <div className={`itemContainer${sliderNo} grid grid-cols-1  md:grid-cols-4 px-7`} id="itemContainer">
                    ...
                    {items.map((item, idx) => {
                        return (<div className='fade hover:scale-[1.1] ease-in-out duration-300' key={item.show.id}>
